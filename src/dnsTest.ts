...
// Mullvad
// const screenshotDir = 'data/stage3/screenshots/' + fullDate + '-mullvad-dnstest'
const csvPath = csvDir + '/' + fullDate + '-mullvad-dnstest.csv'
...
async function runBrowser() {
  ...
  .then(async browser => {
    ...
    for (let j = start; j < end; j++) {
      ...
      const completeDomain = 'https://' + domains[j]
